...
- **Code Snippet Strategy**: Use interactive `{pyodide}` blocks (with `#| autorun: true` and `#| edit: true`) exclusively to show immediate, tweakable results. For "Bug Hunter" and "Guided Practice", use standard static python blocks to push students toward executing code in their own local IDE environments.
