...
- **`### 👨‍💻 Live Coding N: [Name]`**:
  - Done collaboratively by the instructor and students. Use collaborative phrasing ("Ми напишемо", "Створимо").
  - **Контекст**: Scenario or problem description. The context MUST be a pure real-world story. Do NOT explain technical motivation or meta-programming concepts here (e.g., do not say "Instead of writing a for loop, analysts use map"). Just set the scene (e.g., "The analytics department received a large list..."). If the task requires a specific mathematical or business formula, provide the exact formula here. Do NOT provide algorithmic hints here.
